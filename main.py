...
# now use GBM code to calculate portfolio networth in 252 days (1 trading year)
n=252 # full year in trading days
dt=0.000001 # to be somewhat stable in time
gbm= pd.DataFrame()
np.random.seed(1)

def GeometricBrownianMotion(x0, mu, sigma):
...
GBMestimate = np.round_(np.sum(np.multiply(est,prep)),2)
